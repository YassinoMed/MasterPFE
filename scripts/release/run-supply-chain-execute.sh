...
# image scan -> sign -> verify -> promote by digest -> generate SBOM -> attest SBOM -> record evidence.
#
...
if [[ "${SUPPLY_CHAIN_MODE}" == "dry-run" ]]; then
  {
    ...
    printf -- '- Image scanning, signing, verification, promotion, SBOM generation and SBOM attestation were not executed.\n'
  } >> "${SUMMARY_FILE}"
