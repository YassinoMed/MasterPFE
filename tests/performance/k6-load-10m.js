  ...
  scenarios: {
    load_10m: {
      ...
      stages: [
        ...
        { duration: '6m', target: 300 }, // ramp up/sustain to 300 VUs (hundreds of users)
        { duration: '2m', target: 0 },   // ramp down to 0
      ],
      tags: { test_type: 'load_10m', test_name: 'load_10m' },
      gracefulStop: '30s',
    },
  },
  // Use slightly relaxed thresholds for high load to avoid failing prematurely on Kind resource limits
  thresholds: {
    'http_req_duration': ['p(95)<3000'], // 95% of requests must complete under 3s
    'request_errors': ['rate<0.05'],     // error rate must be less than 5%
  },
};
...
export const handleSummary = makeHandleSummary('load_10m', options);
