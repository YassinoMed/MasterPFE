...
| Bloc A | Pods récents, logs, events et healthchecks | TERMINÉ | P0 | Rejouer `make production-runtime-evidence` et `make portal-service-proof` après le dernier déploiement. |
...
| Bloc B | Sécurité post-déploiement runtime | TERMINÉ | P1 | Rejouer `make runtime-security-postdeploy` sur les pods actifs du namespace cible. |
...
| Bloc F | Jenkins webhook / SCM proof | TERMINÉ | P1 | Rejouer `make jenkins-webhook-proof` et `make jenkins-ci-push-proof` sur le Jenkins cible. |
...
- `PARTIEL`: 3
- `PRÊT_NON_EXÉCUTÉ`: 4
- `DÉPENDANT_DE_L_ENVIRONNEMENT`: 2
