...
\hline
\textbf{Test / Script} & \textbf{Attendu} & \textbf{Observe} & \textbf{Statut} \\
\hline
...
\hline
\texttt{validate-dast-report.sh} & Analyse du rapport JSON de scan dynamic OWASP ZAP & Détection et comptage des vulnérabilités, validation de l'absence d'alertes HIGH/CRITICAL & PASS \\
\hline
\texttt{pre-commit run} & Exécution locale des hooks pre-commit (Gitleaks, shellcheck, yaml-check) & Validation statique globale de la qualité et blocage de fuites de secrets avant commit & PASS \\
\hline
\texttt{falcosidekick} & Routage temps réel des détections d'intrusion système & Alertes runtime Falco routées avec succès vers Loki et l'Alertmanager & PASS \\
\hline
