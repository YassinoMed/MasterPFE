...
\begin{itemize}
  ...
  \item \texttt{llm-orchestrator}
  \item \texttt{security-auditor}
  \item \texttt{knowledge-hub}
\end{itemize}

Par la suite, le portail Laravel \texttt{portal-web} a egalement ete integre dans ce schema, afin de ne pas conserver une interface web hors du perimetre DevSecOps.
...
Une fois \texttt{metrics-server} installe, les \textit{HPA} de \texttt{api-gateway} et de \texttt{portal-web} n'affichent plus \texttt{cpu: unknown}. De meme, une fois \texttt{Kyverno} installe, les \textit{ClusterPolicy} du projet sont detectees a l'etat \texttt{Ready}. Ces observations renforcent la credibilite de la plateforme : les manifests ne sont pas seulement versionnes, ils sont egalement activables et observables localement.
