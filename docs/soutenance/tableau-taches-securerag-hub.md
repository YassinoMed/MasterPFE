...
Le projet est opérationnel pour la démonstration applicative et DevSecOps en mode `demo`. Les blocs avancés ne relèvent plus d'un manque de code principal, mais de validations dépendantes de l'environnement d'exécution ou prêtes à être lancées lorsque les prérequis sont disponibles.
