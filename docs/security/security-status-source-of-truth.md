...
- Périmètre officiel / legacy RAG : `TERMINÉ` seulement si `artifacts/final/official-scope-report.md` confirme que le Kustomize officiel référence uniquement les cinq workloads Laravel et que le legacy IA/RAG reste explicitement exclu.
- Jenkins live API/SCM : `TERMINÉ` seulement si `artifacts/validation/jenkins-webhook-proof.md` et `artifacts/validation/jenkins-ci-push-proof.md` annoncent `Status: TERMINÉ`. HTTP 401, 403, 404 ou 000 doivent être classés `DÉPENDANT_DE_L_ENVIRONNEMENT`, pas `PARTIEL` bloquant.
- Kyverno Enforce admission : `TERMINÉ` seulement si `artifacts/validation/kyverno-enforce-proof.md`, `kyverno-admission-positive-test.md` et `kyverno-admission-negative-test.md` prouvent respectivement acceptation d'une image signée par digest et refus d'une image unsafe. Tout rollback doit être archivé dans `artifacts/validation/kyverno-enforce-rollback.md`.
- GitOps Argo CD : `TERMINÉ` seulement si `artifacts/gitops/argocd-sync.md` prouve `Synced` et `Healthy`. Le drift contrôlé doit être séparé dans `artifacts/gitops/drift-proof.md` et protégé par confirmation explicite.
- Observabilité SRE : `TERMINÉ` seulement si les rapports `prometheus-targets.md`, `grafana-dashboard-proof.md`, `loki-logs-proof.md`, `alertmanager-rules.md` et `slo-summary.md` sont présents et cohérents.
- Runtime detection Falco/Tetragon : optionnel ; `PRÊT_NON_EXÉCUTÉ` est acceptable si les ressources VPS/kind ne permettent pas l'installation. Ne pas le rendre bloquant pour la clôture.
