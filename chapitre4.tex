...
\begin{lstlisting}[language=bash, caption={Rapport d'exécution réelle du stage Jenkins Gitleaks (CI/CD)}, label={lst:gitleaks_proof_layer}]
$ gitleaks detect --no-git --config .gitleaks.toml --report-format json --report-path security/reports/gitleaks.json --exit-code 0
INF scan completed in 5m27s
WRN leaks found: 1
+ python3 -c "import json... SARIF 2.1.0 conversion"
+ [ 1 -gt 0 ] -> Quality Gate Triggered (exit 1)
\end{lstlisting}


\paragraph{Expérimentation et Mesures Quantitatives en Pipeline CI/CD.}
L'exécution réelle du stage \textit{Gitleaks Secrets Scan} dans le pipeline Jenkins mesure une durée totale d'analyse de \textbf{5 minutes et 27 secondes} sur l'intégralité du système de fichiers (\texttt{--no-git}). La détection d'une empreinte sensible (\textit{1 leak found}) déclenche l'exécution immédiate d'un script d'invalidation Python. Ce dernier convertit la structure JSON native au format d'échange standardisé \textbf{SARIF v2.1.0} (\texttt{security/reports/gitleaks.sarif}) pour l'intégration au tableau de bord Security Hub, puis retourne un code de sortie strict (\texttt{exit 1}), provoquant l'échec automatique du Quality Gate et le blocage immédiat du déploiement.
